...
class Plate_det(object):
    ...
    def __undistort_new(self,src):   #矫正
        DIM=(640, 480)
        # K=np.array([[437.0084540971927, 0.0, 283.3146555131355], [0.0, 437.6819589909324, 252.65836399811363], [0.0, 0.0, 1.0]])
        # D=np.array([[-0.04663681773493245], [-0.29026347376887063], [0.8395977997602642], [-0.7943376733646794]])
        # K=np.array([[437.29147166828164, 0.0, 283.23918335578566], [0.0, 437.941181602117, 252.88260323287656], [0.0, 0.0, 1.0]])
        # D=np.array([[-0.0524051590976475], [-0.2395079207212681], [0.6876204118634048], [-0.6456909689673381]])
        # K=np.array([[437.50405082668925, 0.0, 284.1759499058224], [0.0, 438.1970867713781, 252.94199295006507], [0.0, 0.0, 1.0]])
        # D=np.array([[-0.05465699944871245], [-0.22728878230186325], [0.6483511354497261], [-0.6017520446699773]])
        ...
        # K=np.array([[436.3561506405744, 0.0, 283.32091012017736], [0.0, 437.22443465429654, 253.317083097084], [0.0, 0.0, 1.0]])
        # D=np.array([[-0.013611066222963921], [-0.49150486855248676], [1.3428920559375706], [-1.2241767699671435]])
        map1, map2 = cv2.fisheye.initUndistortRectifyMap(K, D, np.eye(3), K, DIM, cv2.CV_16SC2)
        __img = cv2.remap(src, map1, map2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
        return __img
